...
const UsersPokemon = ({
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await Promise.all(
          userPokemons.map((url) => fetch(url).then((res) => res.json()))
        );
  ...
  const initialHp = useMemo(() => {
    if (memoized) {
      return playerPoke.stats[0].base_stat;
    }
  }, [memoized]);
    ...
    <div>
      {!pressed ? (
        <div
          ...
          className="carousel slide main-slide"
            ...
            {pokemons.map((pokemon, index) =>
              index === 0 ? (
                <div className="carousel-item active" key={index}>
                  <div className="card main-card" id="playerCard">
                    <img
                    ...
                    <div className="card-body">
                      ...
                      <div className="button">
                        ...
                        >
                          ...
                          <span className="go">Pick me</span>
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        <>
          <div className="card main-card" id="playerCard">
            <img
              src={playerPoke.sprites?.front_default}
              className="d-block w-50 card-img-top"
              alt={playerPoke.name}
            ></img>
            <div className="card-body">
              <h1>{playerPoke.name}</h1>
              <p id="health" className="health-bar">
                HP: {playerPoke.stats[0].base_stat.toFixed(0)}
              </p>
              <p id="health" className="health-bar">
                ATT: {playerPoke.stats[1].base_stat.toFixed(0)}
              </p>
              <p id="health" className="health-bar">
                DEF: {playerPoke.stats[2].base_stat.toFixed(0)}
              </p>
              {playerPoke.stats[0].base_stat.toFixed(2) <= initialHp * 0.3 ? (
                <progress
                  className="nes-progress is-error"
                  value={playerPoke.stats[0].base_stat.toFixed(2)}
                  max={initialHp}
                ></progress>
              ) : (
                <progress
